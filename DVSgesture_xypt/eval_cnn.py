...
class SpikeNN(nn.Module):
    def __init__(self):
        ...
        self.device = torch.device("cuda:0") # or cpu
    ...
    def forward(self, input):
        ...
        #conv1_u = conv1_out = torch.zeros(input.shape[0], 32, 34, 34, device=self.device)
        #pool1_u = pool1_out = torch.zeros(input.shape[0], 32, 17, 17, device=self.device)
        #conv2_u = conv2_out = torch.zeros(input.shape[0], 64, 17, 17, device=self.device)
        #pool2_u = pool2_out = torch.zeros(input.shape[0], 64, 8, 8, device=self.device)
        
        pool1_u = pool1_out = torch.zeros(input.shape[0], 2, 32, 32, device=self.device)
        fc1_u = fc1_out = torch.zeros(input.shape[0], 512, device=self.device)
        fc2_u = fc2_out = spike_sum = torch.zeros(input.shape[0], 11, device=self.device)

        for t in range(step):
            in_t = input[:, :, :, :, t]
            # encoding layer
            conv0_u, conv0_out = state_update(conv0_u, conv0_out, self.conv0(in_t))
            #conv0_out = F.relu(self.conv0(in_t))

            # TODO Auxiliary Update #
            #conv1_u, conv1_out = state_update(conv1_u, conv1_out, self.conv1(conv0_out))
            pool1_u, pool1_out = state_update(pool1_u, pool1_out, F.avg_pool2d(conv0_out, 4))
            #conv2_u, conv2_out = state_update(conv2_u, conv2_out, self.conv2(pool1_out))
            #pool2_u, pool2_out = state_update(pool2_u, pool2_out, F.avg_pool2d(conv2_out, 2))

            # flatten
            #fc_in = pool2_out.view(input.shape[0], -1)
            fc_in = pool1_out.view(input.shape[0], -1)
...
def main():
    ...
    checkpoint_path = './model/gesture_cnn_40.pt'
    if os.path.isdir(checkpoint_path) or True:
        checkpoint = torch.load(checkpoint_path, map_location='cpu')
        model.load_state_dict(checkpoint)
        print('Model loaded.')

    eventflow0 = celexEvalOnce('./data/2.csv', step, dt)
    '''
    eventflow1 = celexEvalOnce('./data/air_guitar.csv', step, dt)
    eventflow2 = celexEvalOnce('./data/arm_roll.csv', step, dt)
    eventflow3 = celexEvalOnce('./data/hand_clapping.csv', step, dt)
    eventflow4 = celexEvalOnce('./data/left_hand_clockwise.csv', step, dt)
    eventflow5 = celexEvalOnce('./data/left_hand_counter_clockwise.csv', step, dt)
    eventflow6 = celexEvalOnce('./data/left_hand_wave.csv', step, dt)
    eventflow7 = celexEvalOnce('./data/other_gestures.csv', step, dt)
    eventflow8 = celexEvalOnce('./data/right_hand_clockwise.csv', step, dt)
    eventflow9 = celexEvalOnce('./data/right_hand_counter_clockwise.csv', step, dt)
    eventflow10 = celexEvalOnce('./data/right_hand_wave.csv', step, dt)
    '''

    start = time.clock()
    test(model, device, eventflow0)
    end = time.clock()
    print(end - start)
    '''
    test(model, device, eventflow1)
    test(model, device, eventflow2)
    test(model, device, eventflow3)
    test(model, device, eventflow4)
    test(model, device, eventflow5)
    test(model, device, eventflow6)
    test(model, device, eventflow7)
    test(model, device, eventflow8)
    test(model, device, eventflow9)
    test(model, device, eventflow10)
    '''
