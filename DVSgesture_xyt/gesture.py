...
class DVSGesture(Dataset):
    def __init__(self, path, train, step, dt):
        ...
        self.eventflow = np.zeros(shape=(self.len, 1, 128, 128, self.step))
        ...
        for num in range(5):
            ...
            for file in files:
                ...
                for i in range(len(win_indices)):
                    ...
                    self.eventflow[filenum, 0, int(all_x[index]), int(all_y[index]), int(all_ts[index]//dt)] = 1  # 1 for an event, 0 for nothing
...
def celexEvalOnce(path, step, dt, genVideo=False):
    ...
    eventflow = np.zeros(shape=(1, 1, 128, 128, step))  # alternative shape
    win_indices = np.where((t < win) & (polar != 0)) 
    win_indices = win_indices[0] # squeeze tuple
    for i in range(len(win_indices)): 
        index = int(win_indices[i])
        #threshold = 2.5
        eventflow[0, 0, int(x[index] // dx), int(y[index] // dy), int(t[index] // dt)] = 1  # 1 for an event, 0 for nothing
